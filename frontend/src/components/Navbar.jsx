...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-40 border-b border-slate-200/50 bg-white/90 backdrop-blur-xl shadow-sm">
      <div className="mx-auto flex max-w-[1600px] items-center gap-3 px-4 py-3 md:px-6 lg:px-8">
        <button onClick={openMobileSidebar} className="grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-slate-50 text-slate-700 transition hover:bg-slate-100 lg:hidden">
          <Menu size={18} />
        </button>

        <div className="min-w-[180px]">
          <p className="text-[10px] uppercase tracking-[0.2em] text-blue-500">AetherCare</p>
          <h1 className="font-heading text-lg font-bold text-slate-900">{pageTitle}</h1>
        </div>

        <div className="ml-4 flex items-center gap-2 rounded-xl bg-gradient-to-r from-slate-900 to-slate-800 px-4 py-2 text-white">
        ...
        <div className="relative ml-auto max-w-xl flex-1" ref={searchRef}>
          <form
            className="flex h-11 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 shadow-sm"
            onSubmit={(e) => {
              e.preventDefault();
          ...
          {searchResults.length > 0 && (
            <div className="absolute left-0 right-0 top-[52px] rounded-xl border border-slate-200 bg-white p-2 shadow-lg">
              {searchResults.map((patient) => (
                ...
                >
                  <span className="text-slate-700">{patient.full_name || `${patient.first_name} ${patient.last_name}`}</span>
        ...
        <div className="flex items-center gap-3">
          <button className="relative grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white text-slate-700 transition hover:bg-slate-100" title="Notifications">
            <Bell size={18} />
          ...
          <div className="relative" ref={dropdownRef}>
            <button
              className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 transition hover:bg-slate-100"
              onClick={() => setDropdownOpen((v) => !v)}
            >
              <div className="grid h-8 w-8 place-items-center rounded-lg bg-blue-100 text-sm font-semibold text-blue-700">
            ...
            {dropdownOpen && (
              <div className="absolute right-0 mt-2 w-56 rounded-xl border border-slate-200 bg-white p-2 shadow-lg">
                ...
                <button className="mt-2 flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-slate-700 hover:bg-slate-50" onClick={() => { setDropdownOpen(false); navigate('/profile'); }}>
                  <User size={15} />
                  Profile
                </button>
                <Link to="/" className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-slate-700 hover:bg-slate-50">
                  Home
                </Link>
                <button className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-red-600 hover:bg-red-50" onClick={handleLogout}>
                  <LogOut size={15} />
                  Sign out
                </button>
              </div>
